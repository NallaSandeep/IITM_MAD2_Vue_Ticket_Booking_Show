    ...
    <section class="jumbotron text-center">
      <div class="container">
        <h2 class="jumbotron-heading">
          <i class="bi bi-film"></i> Reserve Your Spot
        </h2>
      </div>
    </section>
  ...
  methods: {
    async fetchShowList() {
      ...
      const request = new Request(
        "http://localhost:5000/api/shows?page=" +
          this.currentPage +
          "&size=6" +
          dateFilter,
        {
        ...
        .then((response) => {
          if (response.status !== 200) {
            return response.json().then((errorData) => {
              ...
              const errorMessage = errorData.message || "Unable to fetch shows";
    ...
    async fetchShowCount() {
      ...
      const request = new Request(
        "http://localhost:5000/api/shows/pages?size=6" + dateFilter,
        {
          method: "GET",
          headers: headers,
        }
        ...
        .then((response) => {
          if (response.status !== 200) {
            return response.json().then((errorData) => {
              // Extract error message from the response body
              const errorMessage =
                errorData.message || "Unable to fetch shows page count";
              throw new Error(errorMessage);
            });
          }
          return response.json();
